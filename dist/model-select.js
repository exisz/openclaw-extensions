...
import { loadModelPolicy, OCX_MODEL_POLICY_PATH } from "./workspace.js";
export function checkModelPolicy(models) {
...
export function loadOpenClawModels() {
    const raw = execFileSync("openclaw", ["models", "list", "--json"], { encoding: "utf8" });
    const parsed = JSON.parse(raw);
    return parsed.models || [];
}
export function normalizeSubagentTier(value) {
    const policy = loadModelPolicy();
    if (value && policy.subagent[value])
        return value;
    if (value && policy.aliases[value] && policy.subagent[policy.aliases[value]])
        return policy.aliases[value];
    return policy.defaultTier;
}
export function pickSubagentModel(tierInput) {
    const policy = loadModelPolicy();
    ...
    for (const [model, weight] of weights) {
        roll -= weight;
        if (roll <= 0)
            return { model, tier, weights, policyPath: OCX_MODEL_POLICY_PATH };
    }
...
export function runModelSelectCli(args) {
    const cmd = args[0];
