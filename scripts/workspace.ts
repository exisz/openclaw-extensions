...
export const DEFAULT_MODEL_POLICY: Required<ModelPolicyFile> = {
...
function isModelWeight(value: unknown): value is ModelWeight {
  return Array.isArray(value) && typeof value[0] === "string" && typeof value[1] === "number" && value[1] > 0;
}

export function loadModelPolicy(): Required<ModelPolicyFile> {
  ensureOcxWorkspace();
  try {
    const parsed = JSON.parse(readFileSync(OCX_MODEL_POLICY_PATH, "utf-8")) as ModelPolicyFile;
    const subagent: Record<string, ModelWeight[]> = {};
    for (const [tier, weights] of Object.entries(parsed.subagent || {})) {
      const valid = Array.isArray(weights) ? weights.filter(isModelWeight) : [];
      if (valid.length) subagent[tier] = valid;
    }
    return {
