...
function buildDisplayItems(obj) {
    if (obj.type !== 'message')
        return [];
    ...
    if (role === 'user') {
        const text = rawText(content);
        if (text)
            displayItems.push({ kind: 'USER', text, chars: text.length });
    }
...
function printCompactItem(entryNo, item) {
...
function printDetailedItem(entryNo, item) {
    const label = item.name ? `${item.kind} ${item.name}` : item.kind;
    ...
    for (const line of lines) {
        ...
        for (const item of buildDisplayItems(obj)) {
            ...
            if (wanted && (entryNo < wanted[0] || entryNo > wanted[1]))
                continue;
            if (compact)
                printCompactItem(entryNo, item);
            else
                printDetailedItem(entryNo, item);
        }
    }
    if (compact) {
