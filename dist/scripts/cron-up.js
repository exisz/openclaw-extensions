...
/**
 * Adaptive cron frequency adjuster — increase frequency (shorter interval).
 *
 * Usage: ocx cron up <cron_id> [--step N]
 *
...
function main() {
    ...
    if (args.length === 0 || args[0] === "--help" || args[0] === "-h") {
        console.log("Usage: ocx cron up <cron_id> [--step N]");
        console.log("  Increase cron frequency (shorter interval) by N tiers (default 1)");
        process.exit(0);
    }
    const cronId = args[0];
    // Read jobs
    const data = JSON.parse(readFileSync(JOBS_PATH, "utf-8"));
    const job = data.jobs?.find((j) => j.id === cronId);
    if (!job) {
    ...
    // up = move to higher index (shorter interval)
    const newIdx = Math.min(currentIdx + step, TIERS.length - 1);
    if (newIdx === currentIdx) {
        console.log("Already at highest frequency tier.");
    ...
    console.log(`Direction: up (step=${step})`);
}
main();
//# sourceMappingURL=cron-up.js.map