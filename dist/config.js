...
export const SPLIT_SECTIONS = { agents: "agents.json5", bindings: "bindings.json5" };
export function sha256(value) {
    return createHash("sha256").update(value).digest("hex");
}
export function includeRawHash(value) {
    return sha256(`present\0${value}`);
}
export function mergeConfig(sourceConfig) {
    return structuredClone(sourceConfig);
}
export function splitConfig(sourceConfig) {
...
function assertUnchanged(path, originalRaw) {
    const current = readFileSync(path, "utf8");
    if (sha256(current) !== sha256(originalRaw))
        throw new Error(`Config changed concurrently: ${path}`);
}
export function assertIncludeFilesUnchanged(expected = {}) {
    for (const [path, expectedHash] of Object.entries(expected)) {
        ...
        try {
            ...
            throw new Error(`Included config changed or disappeared: ${path}`);
        }
        if (includeRawHash(current) !== expectedHash)
            throw new Error(`Included config changed concurrently: ${path}`);
    }
}
async function loadOpenClawRuntime() {
    ...
    try {
        ...
        try {
            ...
            return await import(pathToFileURL(join(globalRoot, "openclaw/dist/plugin-sdk/config-runtime.js")).href);
        }
        catch {
...
export async function runConfigCommand(argv = process.argv.slice(2)) {
    const action = argv[0];
    ...
    if (configIndex >= 0 && !requestedPath)
        throw new Error("--config requires a path");
    const previousConfig = process.env.OPENCLAW_CONFIG_PATH;
    if (requestedPath)
        process.env.OPENCLAW_CONFIG_PATH = resolve(requestedPath);
    try {
        const runtime = await loadOpenClawRuntime();
        const { snapshot, writeOptions } = await runtime.readConfigFileSnapshotForWrite();
        if (!snapshot.valid)
            throw new Error(`OpenClaw config is invalid: ${(snapshot.issues || []).map(i => i.message).filter(Boolean).join("; ")}`);
        if (requestedPath && resolve(snapshot.path) !== resolve(requestedPath))
            throw new Error(`OpenClaw loaded ${snapshot.path}, not requested ${requestedPath}`);
        const originalRaw = snapshot.raw ?? readFileSync(snapshot.path, "utf8");
        const configDir = dirname(snapshot.path);
        if (action === "merge") {
            if (dryRun) {
                console.log(`Would merge includes into ${snapshot.path}`);
                return;
            }
        ...
        const alreadySplit = Object.entries(SPLIT_SECTIONS).every(([section, file]) => {
            ...
            return value !== null && typeof value === "object" && !Array.isArray(value) &&
                Object.keys(value).length === 1 && value.$include === `./${file}`;
        });
        if (alreadySplit) {
            console.log(`Config is already split: ${snapshot.path}`);
            return;
        }
        ...
        for (const [file, value] of Object.entries(result.files)) {
            ...
            if (existsSync(target)) {
                let existing;
                try {
                    existing = JSON5.parse(readFileSync(target, "utf8"));
                }
                catch {
                    throw new Error(`Refusing to overwrite invalid split target: ${target}`);
                }
                if (JSON.stringify(existing) !== JSON.stringify(value))
                    throw new Error(`Refusing to overwrite non-equivalent split target: ${target}`);
            }
            pending.push([target, content]);
        }
        if (dryRun) {
            console.log(`Would split ${Object.keys(result.files).join(", ")} from ${snapshot.path}`);
            return;
        }
        const backupPath = backup(snapshot.path);
