...
function analyzeTrajectory(filePath) {
    ...
    for (const ev of events) {
        ...
        if (ev.type === 'trace.metadata') {
            ...
            if (!summary.sessionKey)
                summary.sessionKey = ev.sessionKey;
        }
        if (ev.type === 'model.completed') {
            summary.totalCalls++;
            const data = ev.data || {};
            // Check messagesSnapshot for errors
            const msgs = data.messagesSnapshot || [];
            for (const msg of msgs) {
                if (msg.role === 'assistant') {
                    ...
                    if (msg.provider && !summary.provider)
                        summary.provider = msg.provider;
                }
            }
...
function getGatewayErrSnippet(aroundTs, windowMs = 5 * 60 * 1000) {
...
// ── main ───────────────────────────────────────────────────────────────────
function findTrajectoryFiles(agent, channelId) {
...
function printRun(run, showGatewayLog) {
    ...
    if (run.sessionKey)
        console.log(`   Key:     ${run.sessionKey}`);
    ...
    if (showGatewayLog && run.ts) {
        ...
        const relevant = gatewayLines.filter(l => /error|fail|kimi|openrouter|moonshot|model|invalid/i.test(l));
...
//# sourceMappingURL=model-debug.js.map