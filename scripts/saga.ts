...
type Artifact = { source: string; path: string; sha256: string; bytes: number };
...
type SagaState = {
  ...
  saga_version?: string;
  saga_path: string;
  saga_snapshot_path: string;
  saga_sha256: string;
  created_at: string;
  current_stage_index: number;
  status: "running" | "complete" | "abandoned" | string;
...
function slugify(s: string): string { return s.trim().toLowerCase().replace(/[^a-z0-9_.-]+/gi, "-").replace(/^-+|-+$/g, "") || "saga"; }
...
function rel(path: string): string { return relative(ROOT, path) || "."; }
function statePath(runId: string): string { return join(RUN_DIR, `${runId}.json`); }
function asRecord(value: unknown, label: string): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error(`Invalid saga ${label}: expected object`);
  return value as Record<string, unknown>;
}
function requireString(value: unknown, label: string): string {
  if (typeof value !== "string" || !value.trim()) throw new Error(`Invalid saga: ${label} must be a non-empty string`);
  return value;
}
function requireStringList(value: unknown, label: string): string[] | undefined {
...
function validateSaga(data: unknown, source = "<memory>"): Saga {
  const obj = asRecord(data, source);
  const id = requireString(obj.id, "id");
  if (slugify(id) !== id) throw new Error(`Invalid saga ${source}: id must already be a lowercase slug`);
  if (!Array.isArray(obj.stages) || obj.stages.length === 0) throw new Error(`Invalid saga ${source}: requires non-empty stages`);
  for (const key of Object.keys(obj)) if (!ALLOWED_SAGA_KEYS.has(key)) throw new Error(`Invalid saga ${id}: unknown top-level key: ${key}`);
  const seen = new Set<string>();
  const stages: Stage[] = obj.stages.map((raw, i) => {
    const stObj = asRecord(raw, `stage #${i + 1}`);
    for (const key of Object.keys(stObj)) if (!ALLOWED_STAGE_KEYS.has(key)) throw new Error(`Invalid saga ${id} stage #${i + 1}: unknown key: ${key}`);
    const stId = requireString(stObj.id, `stage #${i + 1}.id`);
    if (slugify(stId) !== stId) throw new Error(`Invalid saga ${id} stage #${i + 1}: id must already be a lowercase slug`);
    if (seen.has(stId)) throw new Error(`Invalid saga ${id}: duplicate stage id: ${stId}`);
    seen.add(stId);
    return {
      id: stId,
...
function loadSaga(pathOrName: string): Saga {
  let p = pathOrName;
  if (!existsSync(p)) p = join(SAGA_DIR, `${pathOrName}.json`);
  if (!existsSync(p)) throw new Error(`Saga not found: ${pathOrName}`);
  const saga = validateSaga(JSON.parse(readFileSync(p, "utf-8")), p);
  saga._path = resolve(p);
  return saga;
}
function loadState(runId: string): SagaState {
  const p = statePath(runId);
  if (!existsSync(p)) throw new Error(`Run not found: ${runId}`);
  return JSON.parse(readFileSync(p, "utf-8")) as SagaState;
}
function saveState(state: SagaState): void {
...
function runningStatesForSaga(sagaId: string): SagaState[] {
  mkdirSync(RUN_DIR, { recursive: true });
...
function formatContract(st: Stage): string {
  const parts: string[] = [];
  for (const [key, label] of [["requires", "Requires"], ["produces", "Produces"], ["success_criteria", "Success criteria"]] as const) {
    const values = st[key] || [];
    if (values.length) parts.push(`\n${label}:`, ...values.map((v) => `- ${v}`));
  }
  return parts.join("\n");
}
function priorSubmissionsBlock(state: SagaState): string {
...
function stagePrompt(saga: Saga, idx: number, state: SagaState): string {
...
function getFlag(args: string[], name: string): string | undefined { const i = args.indexOf(name); return i >= 0 ? args[i + 1] : undefined; }
function hasFlag(args: string[], name: string): boolean { return args.includes(name); }
function removeFlagPair(args: string[], name: string): void { const i = args.indexOf(name); if (i >= 0) args.splice(i, 2); }
function copyArtifacts(runId: string, paths: string[]): Artifact[] {
  const out: Artifact[] = [];
  ...
  for (const raw of paths) {
    ...
    if (!existsSync(src) || !statSync(src).isFile()) throw new Error(`Artifact is not a readable file: ${raw}`);
    let dest = join(artifactDir, basename(src));
    if (existsSync(dest)) dest = join(artifactDir, `${basename(src).replace(/(\.[^.]+)?$/, `-${Date.now()}$1`)}`);
...
async function cmdStart(args: string[]): Promise<void> {
  ...
  if (existsSync(statePath(runId))) throw new Error(`Run already exists: ${runId}`);
...
async function cmdSubmit(args: string[]): Promise<void> {
  ...
  if (idx >= saga.stages.length) throw new Error("No current stage; saga already complete");
  ...
  if (!answer) throw new Error("Empty answer rejected");
  const st = saga.stages[idx];
...
function cmdCurrent(args: string[]): void {
  const state = loadState(args[0] || "");
  if (state.status !== "running") { console.log(`Saga is ${state.status}`); return; }
  const saga = loadSaga(join(ROOT, state.saga_snapshot_path));
...
function cmdStatus(args: string[]): void {
  ...
  for (const sub of state.submissions) {
    ...
    for (const art of sub.artifacts) console.log(`  artifact ${art.path} (${art.bytes} bytes, sha256=${art.sha256.slice(0, 12)})`);
  }
}
function cmdList(args: string[]): void {
  ...
  for (const f of readdirSync(RUN_DIR).filter((name) => name.endsWith(".json")).map((name) => join(RUN_DIR, name)).sort((a, b) => statSync(b).mtimeMs - statSync(a).mtimeMs).slice(0, limit)) {
    const s = JSON.parse(readFileSync(f, "utf-8")) as SagaState;
    console.log(`${s.run_id}  ${s.status}  ${s.saga_id}  stage=${s.current_stage_index}`);
  }
}
function cmdSagas(): void {
...
function cmdValidate(args: string[]): void {
  ...
  if (!targets.length) throw new Error("No sagas found");
...
function cmdShow(args: string[]): void {
  ...
  if (saga.principle) console.log(`Principle: ${saga.principle}`);
...
function exportText(state: SagaState, fmt: string): string {
  if (fmt === "json") return `${JSON.stringify(state, null, 2)}\n`;
  const saga = loadSaga(join(ROOT, state.saga_snapshot_path));
  ...
  for (const sub of state.submissions) {
    ...
    for (const art of sub.artifacts) lines.push(`- Artifact: \`${art.path}\` sha256=\`${art.sha256}\``);
...
function cmdExport(args: string[]): void {
  ...
  if (!["markdown", "json"].includes(fmt)) throw new Error("--format must be markdown or json");
  const output = exportText(state, fmt);
  const outPath = getFlag(args, "--output");
  if (outPath) { mkdirSync(dirname(resolve(outPath)), { recursive: true }); writeFileSync(outPath, output); console.log(`exported ${fmt} transcript -> ${outPath}`); }
  else process.stdout.write(output);
}
function cmdAbandon(args: string[]): void {
  const state = loadState(args[0] || "");
  if (state.status !== "running" && !hasFlag(args, "--force")) throw new Error(`Run is not running: ${state.status}. Use --force to mark anyway.`);
  const reason = getFlag(args, "--reason");
  state.status = "abandoned";
  state.abandoned_at = now();
  if (reason) state.notes.push({ at: now(), kind: "abandon_reason", text: reason });
  saveState(state);
